...
while k != len(os.listdir(path_video)):
    ...
    out = cv2.VideoWriter("output.mp4", -1, FPS, (width, height)) #запись нового видео в файл, параметр по дефолту
        ...
        if success == True: #проходим кадры по одному
            ...
            if reader['frame_validity'][i] == 'valid': #если файл доступен
                ...
                for j in range(19, 59, 38): #проход по средним х в csv файле (верхней и нижней точкам оси)
                    ...
                    cv2.line(img, (int(reader.loc[i][19]),int(reader.loc[i][20])), (int(reader.loc[i][57]),int(reader.loc[i][58]-200)), (0, 0, 255), 3)
    ...
    clip.write_videofile('C:/Users/79778/OneDrive/Рабочий стол/Учеба. Бауманка/нирс/videos/РезультатНорма/' + 'new_' + str(name), fps=FPS, codec='libx264')
    k += 1
